#!/usr/bin/env python3
"""Z-Image Turbo + Nunchaku img2img bench — follows the official example pattern.

Auto-detects precision (fp4 on Blackwell 5090). Uses hf://repo/file syntax.
Sweeps rank {32, 128}, resolutions {256, 384, 512}, steps {4, 6, 8} at strength=0.95.
"""
...
# Monkey-patch nunchaku 1.3.0dev's positional super().forward to match current diffusers
...
def run_variant(rank: int, results: list):
    ...
    tag = f"{precision}_r{rank}"
    print(f"\n\n######## {tag} ########", flush=True)

    print(f"[load] transformer from {REPO}/{filename}", flush=True)
    ...
    print("[load] ZImageImg2ImgPipeline", flush=True)
    t0 = time.perf_counter()
    ...
    # Only compile vae.decoder — nunchaku transformer uses its own optimized kernels
...
def main():
    ...
    for r in sorted(results, key=lambda r: r["mean_ms"]):
        print(f"  {r['quant']:12} {r['resolution']:3}² n={r['steps']}: {r['mean_ms']:5}ms ({r['fps']:.2f} fps, {r['vram_gb']}GB)", flush=True)
